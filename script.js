...
// --- CORREÇÃO DO TOAST (MENSAGEM QUE SOME) ---
const toast = (msg) => {
    const t = $('#toast');
    if (!t) return;
    ...
    $('#btnExport').addEventListener('click', () => {
        ...
        win.document.write(`
            ...
            <body>
                ...
                <p><strong>Relatório de Movimentação</strong></p>
                <p>Emissão: ${new Date().toLocaleString('pt-BR')}</p>
                <table>
                    <thead>
                        <tr><th>Nome</th><th>Placa</th><th>Entrada</th><th>Saída</th><th>Porteiro</th></tr>
                    </thead>
                    <tbody>${$('#auditTableBody').innerHTML}</tbody>
                </table>
...
if ($('#btnClearRecords')) {
    $('#btnClearRecords').addEventListener('click', () => {
        ...
        if (confirmar) {
            ...
            renderRecent();
            if ($('#auditTableBody')) filtrarAuditoria();
